...
from .extra_functions import file_processing , convert_to_pdf , random_letters
...
def subject_category (request):
    ...
    if request.method == 'POST':
        ...
        if request.FILES == {} :
            ...
            # Достает имя файла + Рандомизация его имени
            file_name = letters + request.FILES['file'].name 
            ...
            item = SubjectItem.objects.create ( author = request.user.username , title = request.POST ["title"] , theme = request.POST ["theme"],\
                                                ...
                                                docx_file = f'static/workflow/documents/{file_process}' , pdf_file = convert_pdf , \
                                                    
                                                note = request.POST["note"] ,slug = f'{en_slug}{random.choice(string.ascii_letters)}' ,\
        ...
        if en_slug == None :
            ...
            if request.FILES == {} :
                ...
                SubjectItem.objects.filter (slug = subject_item_slug).update ( author = request.user.username , title = request.POST ["title"] ,\
                                                                               ...
                                                                               docx_file = f'static/workflow/documents/{file_process}' , \
                                                                             
                                                                               pdf_file = convert_pdf , note = request.POST["note"] , \
